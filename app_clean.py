# app_clean.py - Clean Working Flask Admin with Manual Sales
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///kaboy_agrovet_clean.db'
...
class OrderItem(db.Model):
    ...
    def __repr__(self):
        return f'<OrderItem {self.id} - {self.quantity}x {self.variant}>'
...
class OfflineSale(db.Model):
    ...
    def __repr__(self):
        return f'<OfflineSale {self.id} - Total: {self.total_cost}>'

    def __str__(self):
        return f'Offline Sale {self.id} - {self.total_cost}'
...
class OfflineSaleItem(db.Model):
    ...
    def __repr__(self):
        return f'<OfflineSaleItem {self.id} - {self.quantity}x {self.variant}>'
...
# --- FLASK ADMIN SETUP ---
admin = Admin(app, name='Kaboy Agrovet Admin', template_mode='bootstrap4', index_view=ManualSaleView(name='Manual Sales', endpoint='/'))
